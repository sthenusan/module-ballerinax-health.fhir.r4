...
# Function to verify whether two patients are same or not in rule based patient matching
#
# + sourcePatient - new patient to be verified  
# + targetPatient - existing patient to be verified 
# + config - configuration details of patientMatcher
# + return - boolean value indicating whether two patients are same or not
public isolated function ruleBasedVerifyPatient(r4:Patient sourcePatient, r4:Patient targetPatient, json config) returns error|http:Response {
    ...
    RulesRecord|error rulesRecord = getPatientMatcherRuleData(config);
...
# Function to match patients in rule based patient matching
#
# + sourcePatient - new patient to be matched 
# + config - configuration details of patientMatcher
# + return - matched patient details or error
public isolated function ruleBasedMatchPatients(r4:Patient sourcePatient, json config) returns error|http:Response {
    record {}|error|() patient = getMatchingPatients(sourcePatient, config);
    if patient is () {
        http:Response response = new;
...
# Function to get MPI database client
#
# + config - configuration details of patientMatcher
# + return - MPI database client or error
public isolated function getMpiDbClient(json config) returns sql:Client|error {

    MPIDbConfig|error dbConfig = getMPIConfigData(config);
...
isolated function getMatchingPatients(r4:Patient patient, json config) returns record {}|error|() {
    stream<record {}, sql:Error?>|error dbPatientStream = getMPIData(patient, check getPatientMatcherRuleData(config), config);
...
isolated function getMPIData(r4:Patient patient, RulesRecord rulesTable, json config) returns stream<record {}, sql:Error?>|error {
    ...
    sql:Client|error dbClient = getMpiDbClient(config);

    if dbClient is error {
        return createPatientMatchingError("Error in getting database client");
    }
    error|string qry = getSQLQuery(patient, rulesTable, config);
    if qry is error {
...
isolated final readonly & PatientMatcherRecord ruleBasedPatientMatcher = {
